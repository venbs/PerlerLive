export const fip = {
    ...
    cartoon:
        ...
        void main() {
            ...
            // 采样步长（即你之前调节的 0.02）
            float d = edgeStrokeWidth; 

            // 计算该点的亮度 (Luminance)
            float c = dot(center.rgb, vec3(0.299, 0.587, 0.114));

            // 采样周边的 8 个点（加上对角线）
            float n  = dot(texture2D(tex0, tc + vec2(0.0, d)).rgb, vec3(0.299, 0.587, 0.114));
            float s  = dot(texture2D(tex0, tc + vec2(0.0, -d)).rgb, vec3(0.299, 0.587, 0.114));
            float e  = dot(texture2D(tex0, tc + vec2(d, 0.0)).rgb, vec3(0.299, 0.587, 0.114));
            float w  = dot(texture2D(tex0, tc + vec2(-d, 0.0)).rgb, vec3(0.299, 0.587, 0.114));
            float nw = dot(texture2D(tex0, tc + vec2(-d, d)).rgb, vec3(0.299, 0.587, 0.114));
            float ne = dot(texture2D(tex0, tc + vec2(d, d)).rgb, vec3(0.299, 0.587, 0.114));
            float sw = dot(texture2D(tex0, tc + vec2(-d, -d)).rgb, vec3(0.299, 0.587, 0.114));
            float se = dot(texture2D(tex0, tc + vec2(d, -d)).rgb, vec3(0.299, 0.587, 0.114));

            // 累计周围所有方向的亮度差绝对值
            float delta = abs(n-c) + abs(s-c) + abs(e-c) + abs(w-c) + abs(nw-c) + abs(ne-c) + abs(sw-c) + abs(se-c);
            
            // 判断是否为边缘（增强后的判断逻辑）
            if (delta > edgeThreshold * 2.0) {
                gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); // 黑色边框
            } else {
                gl_FragColor = center; // 维持原样
            }
        }
    `,
